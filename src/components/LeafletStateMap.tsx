...
export default function LeafletStateMap({ center, districts, liveStats }: StateMapCoreProps) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const map = L.map(divRef.current, { scrollWheelZoom: false }).setView([center.lat, center.lng], 7);
      mapRef.current = map;
      ...
      map.fitBounds(L.latLngBounds(districts.map((d) => [d.lat, d.lng] as [number, number])).pad(0.1));
    })();
